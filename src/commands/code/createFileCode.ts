export function createFile(featureNameSnake: string, classNamePrefix: string) {
    const files = [
        { name: `data/repositories/${featureNameSnake}_repository_impl.dart`, content: `// TODO: ${classNamePrefix} 기능의 리포지토리를 구현하세요.\nclass ${classNamePrefix}RepositoryImpl implements ${classNamePrefix}Repository {}` },
        ...
        { name: `domain/repositories/${featureNameSnake}_repository.dart`, content: `// TODO: ${classNamePrefix} 기능에 대한 리포지토리 인터페이스를 정의하세요.\nabstract class ${classNamePrefix}Repository {}` },
